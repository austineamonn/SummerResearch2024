...
def privatizing_dataset(dataset):
    ...
    private_dataset['race/ethnicity'] = dataset['race/ethnicity'].apply(aggregate_race_ethnicity)

    # Pseudonymizing Gender
    gender_mapping = {"Male": "G1", "Female": "G2"}
    private_dataset['gender'] = dataset['gender'].map(gender_mapping)

    # Pseudonymizing International Status
    international_mapping = {"International": "S1", "Domestic": "S2"}
    private_dataset['international'] = dataset['international'].map(international_mapping)
...
# Aggregating race/ethnicity into broader categories
