...
class DTAlternate:
    ...
    def plotter(self, model=None, save_fig=False, show_fig=False, max_depth=2):
        ...
        if show_fig:
            plt.show()
        if save_fig:
            plt.savefig(f'outputs/{self.privatization_type}/{self.RNN_model}/{self.target_name}/graphs/decision_tree_alternate.png', bbox_inches="tight")
        plt.close()
    ...
    def run_model(self, model=None, ccp_alpha=None, print_report=False, save_files=True, plot_files=True, get_shap=True):
        ...
        if save_files:
            ...
            # Save the model
            self.save_model(f'outputs/{self.privatization_type}/{self.RNN_model}/{self.target_name}/decision_tree_alternate_model.pkl')
